...
class account_loan(osv.osv):
    ...
    def cal_amt(self, cr, uid, ids, vals, context={}):
        ...
        self.write(cr, uid, ids, {'emi_cal': emi, 'tot_amt': tot_amt, 'flat_pa': flat_pa,
                                  'flat_pm': flat_pm, 'tot_int_amt': tot_int_amt, 'yr_int_amt': yr_amt})
        return True

    def re_set(self, cr, uid, ids, vals, context={}):
        self.write(cr, uid, ids, {'loan_amt': 0.00, 'month': 0, 'int_rate': 0.00, 'emi_cal': 0.00,
                                  'tot_amt': 0.00, 'flat_pa': 0.00, 'flat_pm': 0.00, 'tot_int_amt': 0.00, 'yr_int_amt': 0.00})
        return True
    ...
    def create(self, cr, uid, vals, context=None):
        cr_id = super(account_loan, self).create(
            cr, uid, vals, context=context)
        if vals.has_key('loan_amount') and vals['loan_amount']:
            search_ids = self.pool.get('account.loan').search(
                cr, uid, [('loan_id', '=', vals['loan_id'])])
            read_ids = self.pool.get('account.loan').read(
                cr, uid, search_ids, ['loan_amount'])
    ...
    def onchange_partner_id(self, cr, uid, ids, part):
        ...
        addr = self.pool.get('res.partner').address_get(
            cr, uid, [part], ['default', 'invoice'])
        return {'value': {'contact': addr['default']}}

    def onchange_loan_period(self, cr, uid, ids, part):
        sear_list = self.pool.get('loan.installment.period').search(
            cr, uid, [('name', '=', part)])
        value_period = 0
        if sear_list:
            value_period = self.pool.get('loan.installment.period').browse(
                cr, uid, sear_list)[0].period
    ...
    def loan_interest_get(self, cr, uid, ids, context={}):
        ...
        self.pool.get('account.loan').write(cr, uid, ids, {
            'approve_amount': loan[0]['loan_amount'] - loan[0]['process_fee']})
        self.pool.get('account.loan').write(
            cr, uid, ids, {'approve_date': time.strftime('%Y/%m/%d')})
        if loantype != '':
            search_loantype = self.pool.get('account.loan.loantype').search(
                cr, uid, [('name', '=', loan[0]['loan_type'])])
            available_versions = self.pool.get('account.loan.loantype').browse(
                cr, uid, search_loantype[0]).interestversion_ids
            if available_versions:
                search_validversion = self.pool.get('account.loan.loantype.interestversion').search(cr, uid, [('loantype_id', '=', search_loantype[
                    0]), ('start_date', '<=', loan[0]['apply_date']), ('end_date', '>=', loan[0]['apply_date'])])
                if search_validversion:
                    available_versionlines = self.pool.get('account.loan.loantype.interestversion').browse(
                        cr, uid, search_validversion[0]).interestversionline_ids
                    if available_versionlines:
                        search_validversionlines = self.pool.get('account.loan.loantype.interestversionline').search(cr, uid, [('interestversion_id', '=', search_validversion[0]), ('min_month', '<=', loan[
                            0]['total_installment']), ('max_month', '>=', loan[0]['total_installment']), ('min_amount', '<=', loan[0]['loan_amount']), ('max_amount', '>=', loan[0]['loan_amount'])])
                        if search_validversionlines:
                            valid_versionline = self.pool.get('account.loan.loantype.interestversionline').browse(
                                cr, uid, search_validversionlines[0]).rate
                            if valid_versionline:
                                self.pool.get('account.loan').write(
                                    cr, uid, ids, {'interest_rate': valid_versionline})
                            else:
                                raise osv.except_osv(
                                    'Information Required', 'Interest Rate is Not Defined')
                        else:

                            raise osv.except_osv(
                                'Information Required', 'There is not valid interest versionlines availble. so please check it')
                    else:
                        raise osv.except_osv(
                            'Information Required', 'There is no versions Defined.')
                else:
                    raise osv.except_osv(
                        'Information Required', 'Interest version is Not Defined')
            else:
                raise osv.except_osv('Information Required',
                                     'Interest version is Not Defined')
        else:
            raise osv.except_osv('Information Required',
                                 'Loan Type is Not Defined')
    ...
    def _simple_interest_get(self, cr, uid, ids, inter_rate):
        ...
        if loan[0]['return_type'] == 'cheque':
            ...
            bank_ids = self.pool.get('res.partner.bank').search(
                cr, uid, [('partner_id', '=', part_id)])
            if bank_ids:
                acc_ids = self.pool.get('account.account').search(
                    cr, uid, [('id', '=', bank_acc)])
            else:
                raise osv.except_osv(
                    'Field Required', 'Please select Bank Account For ' + loan[0]['partner_id'][1])

        inter_sed = self.pool.get('account.loan.loantype').search(
            cr, uid, [('name', '=', loan[0]['loan_type'])])
        if inter_sed:
            inter_cal_type = self.pool.get('account.loan.loantype').browse(
                cr, uid, inter_sed[0]).calculation
            if inter_cal_type:
        ...
        installment = round(((total * (rate_interest / 12)) / (1 -
                                                               ((1 + rate_interest / 12)**-(loan[0]['total_installment'])))))
        i = 1
        j = 1
        interest = 0
        ...
        while i <= loan[0]['total_installment']:
            ...
            if loan[0]['return_type'] == 'cheque':
                ...
                j += 1
                cheque_cr.create(cr, uid, {'name': 'cheque' + str(i), 'date': date_update, 'loan_id': ids[0], 'code': 'cheque' + str(i), 'partner_id': part_id, 'loan_bank_id': bank_ids[
                                 0], 'account_id': acc_ids[0], 'cheque_amount': installment, 'loan': principle_amount, 'interest': interest_month})
            ...
            for read_id in read_ids:
                chq = read_id['id']
            installment_cr.create(cr, uid, {'name': 'installment' + str(i), 'loan_id': ids[
                                  0], 'capital': principle_amount, 'interest': interest_month, 'total': installment, 'cheque_id': chq})
            total = remain_amount
            interest += interest_month
            i += 1
        self.pool.get('account.loan').write(
            cr, uid, ids, {'interest': interest})
        ...
        period_id = pool.get('account.period').search(
            cr, uid, [('date_start', '<=', curr_date), ('date_stop', '>=', curr_date)])[0]
        journal_id = pool.get('account.journal').search(
            cr, uid, [('name', '=', 'x Bank Journal')])[0]
        move = {'name': '/', 'journal_id': journal_id,
                'period_id': period_id, 'date': curr_date}
        move_id = self.pool.get('account.move').create(cr, uid, move)
        acc_move_line_name = emp_name

        try:
            pool.get('account.move.line').create(cr, uid, {'move_id': move_id, 'acc_loan_id': ids[
                0], 'name': acc_move_line_name, 'date': curr_date, 'account_id': sal_obj.proc_fee.id, 'credit': pr_fee, 'debit': 0.0, 'journal_id': 5, 'period_id': period_id})
            pool.get('account.move.line').create(cr, uid, {'move_id': move_id, 'acc_loan_id': ids[
                0], 'name': acc_move_line_name, 'date': curr_date, 'account_id': sal_obj.partner_id.property_account_payable.id, 'credit': amt, 'debit': 0.0, 'journal_id': 5, 'period_id': period_id})
        except:
            raise osv.except_osv(
                'Error', 'Could not create account move lines. ')

        try:
            pool.get('account.move.line').create(cr, uid, {'move_id': move_id, 'acc_loan_id': ids[
                0], 'name': acc_move_line_name, 'date': curr_date, 'account_id': sal_obj.cus_pay_acc.id, 'debit': gross, 'credit': 0.0, 'journal_id': 5, 'period_id': period_id})
        except:
            raise osv.except_osv(
                'Error', 'Could not create account move line. ')
    ...
    def reject_loan(self, cr, uid, ids, context={}):
        ...
        installment_cr = self.pool.get('account.loan.installment')
        install_ids = installment_cr.search(
            cr, uid, [('loan_id', '=', ids[0])])
    ...
    def proof_approval(self, cr, uid, ids, context={}):
        ...
        if loantype != '':
            search_loantype = self.pool.get('account.loan.loantype').search(
                cr, uid, [('name', '=', loantype)])
            prooftype_req = self.pool.get('account.loan.loantype').browse(
                cr, uid, search_loantype[0]).prooftypes
            for pt in prooftype_req:
                avail_proof_docs = self.pool.get('account.loan.proof').search(cr, uid, [(
                    'loan_id', '=', ids[0]), ('type', '=', pt.name), ('state', '=', 'done')])
                if not avail_proof_docs:
                    raise osv.except_osv('Field Required', 'This loan object has not valid ' +
                                         pt.name + ' proofs so please submit proof again and valid it')
            self.pool.get('account.loan').write(
                cr, uid, ids, {'state': 'apply'})
        else:
            raise osv.except_osv('Information Required',
                                 'Loan Type is Not Defined')
    ...
    def apply_loan(self, cr, uid, ids, context={}):
        ...
        if inter_sed:
            ...
            if inter_cal_type:
                if inter_cal_type == 'flat':
                    interest_rate1 = self.pool.get(
                        'account.loan')._simple_interest_get(cr, uid, ids, inter_rate)
                elif inter_cal_type == 'reducing':
                    interest_rate1 = self.pool.get(
                        'account.loan')._simple_interest_get(cr, uid, ids, inter_rate)
            else:
                raise osv.except_osv(
                    'Field Required', 'Interest Type Not Selected')
        else:
            ...
            if voucher_state and voucher_state in voucher_dict:
                for voucher_method in voucher_dict[voucher_state]:
                    getattr(voucher_pool, voucher_method)(
                        cr, uid, [this_obj.voucher_id.id], context=context)

            move_ids = voucher_obj and [
                x.id for x in voucher_obj.move_ids] or []

            if move_ids:
                self.pool.get('account.move.line').write(
                    cr, uid, move_ids, {'acc_loan_id': this_obj.id})

        return True

    def unlink(self, cr, uid, ids, context=None):
        vouchers = self.pool.get('account.voucher').read(
            cr, uid, ids, ['state'])
        unlink_ids = []
        for t in vouchers:
            if t['state'] in ('draft', 'cancel'):
                ...
                raise osv.except_osv(_('Invalid action !'), _(
                    'Cannot delete vouchers(s) which are already opened or paid !'))
