...
# account_loan_installment.loan_id=account_loan.id and account_loan.id="
# +str(loan.id)+" group by account_loan_installment.loan_id")
    ...
    def __ending_date__(self, loan):
        ...
        j = 12
        if j == total_inst:
            end_date = mx.DateTime.strptime(
                start_date, '%Y-%m-%d') + RelativeDateTime(days=i)
        else:
            while j < total_inst:
                j = j + 12
                i = i + 365
