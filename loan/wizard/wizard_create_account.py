...
class wizard_cus_acct(wizard.interface):
    ...
    def get_defaults(self, cr, uid, data, context):
        user = pooler.get_pool(cr.dbname).get(
            'account.loan').browse(cr, uid, data['id'])
    ...
    def create_cus_acct(self, cr, uid, data, context):
        ...
        name = user.read(cr, uid, [data['id']], ['partner_id'])[
            0]['partner_id']
        ...
        user.write(cr, uid, data['id'], {
                   'cus_pay_acc': credit_id, 'int_acc': dname, 'bank_acc': bnk_acc, 'proc_fee': prc_fee})
